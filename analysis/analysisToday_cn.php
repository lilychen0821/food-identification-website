...
function requirementNlimit($age,$gender)//caculate range of vitamin and minerals intake
{
	...
	if($gender=="male")
	{
		$vitaminsR=array(0.9/1000,16/1000,90/1000,0.015/1000,15/1000);
		$vitaminsL=array(3/1000,1000,2,0.1/1000,1);
		if($age<31)
		{
	...
	{
		$vitaminsR=array(0.7/1000,0.014,0.075,0.015/1000,0.015);
		$vitaminsL=array(0.003,1000,2,0.1/1000,1);
		if($age<31)
		{
...
function ifvNmOvertaken($intook,$limit)//check if vitamins intook is too much
{
	$temp=[];
	for($i=0;$i<count($intook);$i++)
	{
		if($intook[$i]>$limit[$i])
		{
			$temp=$i;
...
<a href="analysisToday.php" class="text-right pull-right">English</a>
<?php
//-------------------------------------------------------------main()
//if login 
if((!isset($_SESSION["id"]))||(!isset($_SESSION["account"])))
{
	alert("—— 需要先登入才能進行分析哦 ——","../../index.php");
}
else// login statement
{
	...
	if($crecord['recordcheck']=="true")//check if finished bmi info
	{
		...
		{
			echo "</br>今天還沒有飲食分析紀錄哦！";
		}
		else//if had recorded
		{
			foreach($todayrows as $ttoday)//get all intook nutrients 
			{
			...
			//----check if vitamin and minerals overtaken
			...
			$ifv2much=ifvNmOvertaken($vNm,$vitaminL);
			if($ifv2much!=0)
			{
		...
		//-----------------------draw charts
			...
			$dataPointsChart2 = array(
					array("label"=> "攝取量", "y"=> $sumarr['0'],"cmt"=>$energycmt),
					array("label"=> "需求量", "y"=> $dailyenergy['0'],"cmt"=>$energycmt)
				);		
			
			//for chart1 ——> bar chart
					$dataPointsBar1 = array(
						array("label"=> "蛋白質", "y"=> $sumarr['1']*4),
						array("label"=> "碳水化合物", "y"=> $sumarr['2']*4),
						array("label"=> "脂肪", "y"=> $sumarr['3']*9)
					);
					$dataPointsBar2 = array(
						array("label"=> "蛋白質", "y"=> $dailyenergy['1']),
						array("label"=> "碳水化合物", "y"=> $dailyenergy['2']),
						array("label"=> "脂肪", "y"=> $dailyenergy['3'])
					);
			//---end of bar chart
		//---------------------end of draw charts
?>
...
{
	alert("—— 要先完成 BMI 資料填寫才能進行分析哦 ——","../index.php");
}
